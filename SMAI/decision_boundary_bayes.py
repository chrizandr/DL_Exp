...
def decision_boundary(means, covariances, probs):
    ...
    w2_k1 = np.dot(inverse_covariance[k1], means[k1])
    w0_k1 = -0.5*np.dot(np.dot(np.transpose(means[k1]), inverse_covariance[k1]), means[k1]) + \
             np.log(probs[k1]) - 0.5*np.log(np.linalg.norm(covariances[k1]))
    ...
    w2_k2 = np.dot(inverse_covariance[k2], means[k2])
    w0_k2 = -0.5*np.dot(np.dot(np.transpose(means[k2]), inverse_covariance[k2]), means[k2]) + \
             np.log(probs[k2]) - 0.5*np.log(np.linalg.norm(covariances[k2]))
